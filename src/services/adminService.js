...
let getWeeklyRevenue = () => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            invoices = invoices.filter((item) => item.createdAt >= sixDaysAgo && item.createdAt <= currentDate);
        ...
        try {
            ...
            if (threeDoctors) {
                threeDoctors.map(async (item) => {
                    let resRevenueEachMonthOfDoctor = await getTotalRevenueDoctorEachMonthByDoctorId(item.doctorId);
                    if (resRevenueEachMonthOfDoctor && resRevenueEachMonthOfDoctor.errCode === 0) {
                        ...
                        let copy_RevenueEachMonthOfThreeDoctors = [...RevenueEachMonthOfThreeDoctors];
                        RevenueEachMonthOfThreeDoctors = [...copy_RevenueEachMonthOfThreeDoctors, dataRevenueDoctor];
...
let getMonthlyRevenueSpecialty = () => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            invoices = invoices.filter((item) => item.createdAt >= startOfMonth && item.createdAt <= endOfMonth);
