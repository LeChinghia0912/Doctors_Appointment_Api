...
let handleUserLogin = (email, password) => {
    return new Promise(async (resolve, reject) => {
        try {
            let userData = {};
            // lkiểm tra sự tồn tại của 1 địa chỉ email
            let isExist = await checkUserEmail(email);
            if (isExist) {
                let user = await db.User.findOne({
                    where: { email: email },
                    attributes: [
                        // danh sách thuộc tính lấy từ bảng
                        'id',
                        'email',
                        'roleId',
                        'password',
                ...
                if (user) {
                    ...
                    if (check) {
                        ...
                        delete user.password; // đảm bảo thông tin mật khẩu không được trả về phía client
                        userData.user = user;
                    } else {
                        userData.errCode = 3;
                    ...
                    if (user && user.status && user.status != 0) {
                        // nếu user không tồn tại hoặc trạng thái ==0 // => người dùng không hoạt động
                        userData.errCode = 1;
                ...
                userData.errCode = 1; // không có email trong hệ thống
        ...
        try {
            ...
            if (user) {
                currentUser = await db.User.findOne({
                    ...
                    attributes: ['id', 'email', 'roleId', 'password', 'firstName', 'lastName', 'image', 'address', 'gender', 'phonenumber'],
                    include: [
                        {
                            model: db.Doctor_Infor,
                            attributes: ['priceId', 'specialtyId'],
